...
library(MASS)
library(Matrix)
my.filepath = "/Users/shixu/Dropbox/Word2vec/Report/W2V_revision/UploadCodeJASA/SaveToSelf_revision_Supp_Section_G/"
rowNorms = function(M){
...
run_uneven_mod <- function(numberofgroups,seed,kappa){
  ...
  for(i in 1:n){
    ...
    if(n.shuffle>0){
      ...
      }#end for(j in 1:n.shuffle)
    }#end if(n.shuffle>0)
  }#end for(i in 1:n){
  
  ########################################
  S = length(which(diag(A)!=1)) ##how many mismatch
  N-S;S/N ## how many match
  X.save=X=X.mean
  Y.mean = A%*%X%*%W.true
